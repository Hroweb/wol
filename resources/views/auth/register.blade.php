...
{{--    <x-auth-validation-errors class="mb-4" :errors="$errors" />--}}
    ...
    <div class="flex w-full min-h-screen">
        ...
        <div class="w-full md:w-1/2 flex flex-col items-center justify-center p-6">
            <form method="POST" action="{{ route('register') }}" class="w-80 md:w-96 flex flex-col items-center">
                @csrf
                ...
                <div class="w-full mt-6">
                    <x-input-label for="surname" :value="__('Surname')" class="sr-only" />
                    <div
                        class="flex items-center w-full bg-transparent border border-gray-300/60 h-12 rounded-full overflow-hidden pl-6 gap-2">
                        <x-text-input id="surname" name="surname" type="text" :value="old('surname')" required
                                      placeholder="Surname"
                                      class="bg-transparent text-gray-700 placeholder-gray-500/80 outline-none text-sm w-full h-full border-0 focus:ring-0" />
                    </div>
                ...
                <!-- Submit -->
                <x-primary-button
                    class="mt-8 w-full h-11 rounded-full text-white bg-[#101828] hover:opacity-90 transition-opacity justify-center">
                    {{ __('Sign up') }}
                </x-primary-button>
                ...
                <p class="text-gray-500/90 text-sm mt-4">
                    ...
                    <a class="text-[#101828] hover:underline" href="{{ route('login') }}">
                        {{ __('Sign in') }}
                    </a>
                </p>
            </form>
